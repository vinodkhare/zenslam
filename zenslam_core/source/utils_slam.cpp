...
void zenslam::utils::correspondences_3d2d(
    ...
    std::vector<cv::Point3d> &points3d, std::vector<cv::Point2d> &points2d,
...
void zenslam::utils::correspondences_3d3d(
    ...
    std::vector<cv::Point3d> &points3d_0, std::vector<cv::Point3d> &points3d_1,
...
auto zenslam::utils::estimate_pose_3d2d(
    ...
    const cv::Matx33d &camera_matrix, const double &threshold) -> pose_data {
  std::vector<cv::Point3d> points3d;
  std::vector<cv::Point2d> points2d;
  std::vector<size_t> indices = {};
  correspondences_3d2d(map_points_0, map_keypoints_1, points3d, points2d,
                       indices);
  ...
  if (points3d.size() >= 6) {
    ...
    if (cv::solvePnPRansac(points3d, points2d, camera_matrix, cv::Mat(), rvec,
                           tvec, true, 1000, gsl::narrow<float>(threshold),
                           0.99, inliers)) {
      ...
      auto inliers_set =
          std::set(pose_data.inliers.begin(), pose_data.inliers.end());
      for (auto i : indices)
        if (!inliers_set.contains(i))
          pose_data.outliers.push_back(i);

      auto points2d_back = std::vector<cv::Point2d>{};
      cv::projectPoints(points3d, rvec, tvec, camera_matrix, cv::Mat(),
                        points2d_back);
  ...
  throw std::runtime_error(
      "Not enough 3D-2D correspondences to compute pose (need > 6)");
}

auto zenslam::utils::estimate_pose_3d3d(
    const std::map<size_t, point3d> &map_points_0,
    const std::map<size_t, point3d> &map_points_1, const double &threshold)
    -> pose_data {
  // Gather points from slam.frame[0] and slam.frame[1] for 3D-3D pose
  // computation
  ...
  // Compute relative pose between slam.frame[0] and slam.frame[1] using 3D-3D
  // correspondences
  if (points_0.size() >= 3) {
    cv::Matx33d R;
    cv::Point3d t;
    ...
    estimate_rigid_ransac(points_0, points_1, R, t, inliers, outliers, errors,
                          threshold, 1000);
  ...
  throw std::runtime_error(
      "Not enough 3D-3D correspondences to compute pose (need > 3)");
}

auto zenslam::utils::estimate_pose(
    const std::map<size_t, point3d> &map_points_prev,
    const frame::tracked &current, const calibration &calibration,
    const class options::slam &options) -> estimate_pose_result {
  estimate_pose_result result{};

  // Attempt 3D-3D
  try {
    result.pose_3d3d = estimate_pose_3d3d(map_points_prev, current.points3d,
  ...
  try {
    result.pose_3d2d = estimate_pose_3d2d(map_points_prev, current.keypoints[0],
  ...
  // Choose the pose with more inliers; if tie, prefer 3D-3D; if both invalid,
  // identity
...
bool zenslam::utils::estimate_rigid(const std::vector<cv::Point3d> &points3d_0,
  ...
  // Compute means
  auto mean_src =
      std::accumulate(points3d_0.begin(), points3d_0.end(), cv::Vec3d(0, 0, 0),
  ...
  auto mean_dst =
      std::accumulate(points3d_1.begin(), points3d_1.end(), cv::Vec3d(0, 0, 0),
                      [](const cv::Vec3d &acc, const cv::Point3d &p) {
                        return acc + cv::Vec3d(p.x, p.y, p.z);
                      }) /
  ...
  for (size_t i = 0; i < points3d_0.size(); ++i) {
    auto a =
        cv::Vec3d(points3d_0[i].x, points3d_0[i].y, points3d_0[i].z) - mean_src;
    auto b =
        cv::Vec3d(points3d_1[i].x, points3d_1[i].y, points3d_1[i].z) - mean_dst;
    Sigma(0, 0) += b[0] * a[0];
    Sigma(0, 1) += b[0] * a[1];
    Sigma(0, 2) += b[0] * a[2];
    Sigma(1, 0) += b[1] * a[0];
...
auto zenslam::utils::predict_pose_from_imu(const cv::Affine3d &pose_0,
...
auto zenslam::utils::estimate_rigid_ransac(
    const std::vector<cv::Point3d> &src, const std::vector<cv::Point3d> &dst,
    cv::Matx33d &best_R, cv::Point3d &best_t,
    std::vector<size_t> &inlier_indices, std::vector<size_t> &outlier_indices,
    std::vector<double> &errors, const double threshold,
...
auto zenslam::utils::filter(const std::vector<cv::KeyPoint> &keypoints0,
...
auto zenslam::utils::create_matcher(const class options::slam &options,
                                    bool is_binary)
...
auto zenslam::utils::match_keypoints(
    const map<keypoint> &keypoints_0, const map<keypoint> &keypoints_1,
    const cv::Ptr<cv::DescriptorMatcher> &matcher,
    const class options::slam &options) -> std::vector<cv::DMatch> {
  ...
  if (use_ratio_test) {
    ...
    for (const auto &knn : knn_matches) {
      if (knn.size() == 2 &&
          knn[0].distance < options.matcher_ratio * knn[1].distance) {
  ...
  if (matches.size() >= 8) {
    auto points_l = matches |
                    std::views::transform([&unmatched_l](const auto &match) {
                      return unmatched_l[match.queryIdx].pt;
                    }) |
                    std::ranges::to<std::vector>();

    auto points_r = matches |
                    std::views::transform([&unmatched_r](const auto &match) {
    ...
    cv::Mat fundamental =
        cv::findFundamentalMat(points_l, points_r, cv::FM_RANSAC,
                               options.epipolar_threshold, 0.99, inlier_mask);
  ...
  for (const auto &match : matches) {
    ...
    matches_new.emplace_back(keypoint_l.index, keypoint_r.index,
                             match.distance);
  }
...
auto zenslam::utils::match_keypoints3d(
    const point3d_cloud &points3d_world, const map<keypoint> &keypoints,
    const cv::Affine3d &pose_of_camera0_in_world, const cv::Matx34d &projection,
    const double radius, const double threshold) -> std::vector<cv::DMatch> {
  ...
  const auto &points3d =
      pose_of_camera0_in_world.inv() *
          points3d_world.radius_search(
  ...
  cv::vconcat(points3d | std::views::transform([](const auto &kp) {
  ...
  cv::vconcat(untriangulated | std::views::transform([](const auto &keypoint) {
  ...
  cv::BFMatcher(descriptors3d.depth() == CV_8U ? cv::NORM_HAMMING : cv::NORM_L2,
                true)
  ...
  for (auto i = 0; i < matches_cv.size(); ++i) {
    ...
    {
      matches.emplace_back(matched_points3d[i].index,
                           matched_keypoints[i].index, matches_cv[i].distance);
    }
  }
...
auto zenslam::utils::match_keylines(const map<keyline> &keylines_map_0,
  ...
  for (const auto &match : matches) {
    ...
    std::vector pts0 = {cv::Point2f(kl0.startPointX, kl0.startPointY),
                        cv::Point2f(kl0.endPointX, kl0.endPointY),
                        cv::Point2f(kl0.pt.x, kl0.pt.y)};

    std::vector pts1 = {cv::Point2f(kl1.startPointX, kl1.startPointY),
    ...
    for (auto i = 0; i < 3; ++i) {
      ...
      double err0 = std::abs(epilines0[i][0] * pts0[i].x +
                             epilines0[i][1] * pts0[i].y + epilines0[i][2]) /
                    std::sqrt(epilines0[i][0] * epilines0[i][0] +
                              epilines0[i][1] * epilines0[i][1]);

      // Error for pt1 to epiline in image 1
      double err1 = std::abs(epilines1[i][0] * pts1[i].x +
                             epilines1[i][1] * pts1[i].y + epilines1[i][2]) /
                    std::sqrt(epilines1[i][0] * epilines1[i][0] +
...
auto zenslam::utils::match_temporal(
    ...
    const cv::Matx33d &camera_matrix, double threshold)
    -> std::vector<cv::DMatch> {
  ...
  const cv::BFMatcher matcher{
      descriptors_0.depth() == CV_8U ? cv::NORM_HAMMING : cv::NORM_L2, true};
  std::vector<cv::DMatch> matches;
  matcher.match(descriptors_0, descriptors_1, matches);

  // Filter matches based on reprojection error
  auto points_0 = matches |
                  std::views::transform([&unmatched_0](const auto &match) {
                    return unmatched_0[match.queryIdx].pt;
                  }) |
                  std::ranges::to<std::vector>();

  auto points_1 = matches |
                  std::views::transform([&unmatched_1](const auto &match) {
  ...
  auto e = cv::findEssentialMat(points_0, points_1, camera_matrix, cv::RANSAC,
                                0.99, threshold, mask);
...
auto zenslam::utils::process(const frame::sensor &sensor,
  ...
  std::jthread thread_0{[&]() {
    ...
    processed.undistorted[0] = rectify(
        processed.images[0], calibration.map_x[0], calibration.map_y[0]);
  ...
  std::jthread thread_1{[&]() {
    ...
    processed.undistorted[1] = rectify(
        processed.images[1], calibration.map_x[1], calibration.map_y[1]);
...
auto zenslam::utils::solve_pnp(const cv::Matx33d &camera_matrix,
  ...
  if (cv::solvePnPRansac(points3d, points2d, camera_matrix, cv::Mat(), rvec,
                         tvec, true, 1000, 4.0, 0.99, inliers)) {
    SPDLOG_INFO("SolvePnP successful with {} inliers out of {} points",
...
auto zenslam::utils::track(const frame::tracked &frame_0,
  ...
  {
    std::jthread thread_0{[&]() {
      keypoints_0 += track_keypoints(frame_0.pyramids[0], frame_1.pyramids[0],
                                     frame_0.keypoints[0], options,
                                     calibration.camera_matrix[0]);
    ...
    std::jthread thread_1{[&]() {
      keypoints_1 += track_keypoints(frame_0.pyramids[1], frame_1.pyramids[1],
                                     frame_0.keypoints[1], options,
  ...
  points3d += triangulate_keypoints(
      keypoints_0, keypoints_1, calibration.projection_matrix[0],
...
auto zenslam::utils::track_keypoints(
    ...
    const std::vector<cv::Mat> &pyramid_1, const map<keypoint> &keypoints_map_0,
    const class options::slam &options, const cv::Matx33d &camera_matrix,
    const std::vector<cv::Point2f> &points_1_predicted)
    -> std::vector<keypoint> {
  // Convert previous keypoints to Point
  const auto &keypoints_0 =
      keypoints_map_0.values() | std::ranges::to<std::vector>();
  ...
  cv::calcOpticalFlowPyrLK(
      pyramid_0, pyramid_1, points_0, points_1, status, errors,
      options.klt_window_size, options.klt_max_level,
      cv::TermCriteria(cv::TermCriteria::COUNT | cv::TermCriteria::EPS, 99,
                       0.001),
  ...
  cv::calcOpticalFlowPyrLK(
      pyramid_1, pyramid_0, points_1, points_0_back, status_back, errors,
      options.klt_window_size, options.klt_max_level,
      cv::TermCriteria(cv::TermCriteria::COUNT | cv::TermCriteria::EPS, 99,
                       0.001),
      cv::OPTFLOW_LK_GET_MIN_EIGENVALS);
  ...
  assert(points_0.size() == points_1.size() &&
         points_1.size() == status.size() && status.size() == errors.size());
  ...
  for (size_t i = 0; i < points_1.size(); ++i) {
    if (status[i] && status_back[i] &&
        cv::norm(points_0_back[i] - points_0[i]) < options.klt_threshold) {
  ...
  if (candidate_points_0.size() >= 8) {
    ...
    auto E = cv::findEssentialMat(candidate_points_0, candidate_points_1,
                                  camera_matrix, cv::RANSAC, 0.999,
                                  // Confidence
...
auto zenslam::utils::track_keylines(const std::vector<cv::Mat> &pyramid_0,
  ...
  cv::calcOpticalFlowPyrLK(
      pyramid_0, pyramid_1, start_points_0, start_points_1, status_start_fwd,
      err_start_fwd, options.klt_window_size, options.klt_max_level,
      cv::TermCriteria(cv::TermCriteria::COUNT | cv::TermCriteria::EPS, 99,
                       0.001),
      cv::OPTFLOW_LK_GET_MIN_EIGENVALS);

  cv::calcOpticalFlowPyrLK(
      pyramid_0, pyramid_1, end_points_0, end_points_1, status_end_fwd,
      err_end_fwd, options.klt_window_size, options.klt_max_level,
      cv::TermCriteria(cv::TermCriteria::COUNT | cv::TermCriteria::EPS, 99,
                       0.001),
      cv::OPTFLOW_LK_GET_MIN_EIGENVALS);
  ...
  cv::calcOpticalFlowPyrLK(
      pyramid_1, pyramid_0, start_points_1, start_points_0_back,
      status_start_bwd, err_start_bwd, options.klt_window_size,
  ...
  cv::calcOpticalFlowPyrLK(
      pyramid_1, pyramid_0, end_points_1, end_points_0_back, status_end_bwd,
      err_end_bwd, options.klt_window_size, options.klt_max_level,
      cv::TermCriteria(cv::TermCriteria::COUNT | cv::TermCriteria::EPS, 99,
                       0.001),
      cv::OPTFLOW_LK_GET_MIN_EIGENVALS);
  ...
  for (size_t i = 0; i < keylines_0.size(); ++i) {
    ...
    if (status_start_fwd[i] && status_end_fwd[i] && status_start_bwd[i] &&
      ...
      const auto fb_error_start = static_cast<float>(
          cv::norm(start_points_0_back[i] - start_points_0[i]));
      ...
      // threshold
      if (fb_error_start < options.klt_threshold &&
          fb_error_end < options.klt_threshold) {
        ...
        tracked_keyline.angle =
            std::atan2(dy, dx) * 180.0f / static_cast<float>(CV_PI);
...
auto epipolar_angles(const cv::Vec3d &translation_of_camera1_in_camera0) {
  return std::views::transform([&translation_of_camera1_in_camera0](
                                   const cv::Point3d &p) {
    ...
    return norm_prod < 1e-12
               ? 0.0
               : std::abs(std::acos(
                     std::clamp(vec_to_point_0.dot(vec_to_point_1) / norm_prod,
                                -1.0, 1.0))) *
                     180 / CV_PI;
  });
}
...
auto zenslam::utils::triangulate_keypoints(
    const map<keypoint> &keypoints_0, const map<keypoint> &keypoints_1,
    const cv::Matx34d &projection_0, const cv::Matx34d &projection_1,
    const double triangulation_threshold,
    const cv::Vec3d &translation_of_camera1_in_camera0)
    -> std::vector<point3d> {
  const auto &points2f_0 = to_points(keypoints_0.values_matched(keypoints_1) |
                                     std::ranges::to<std::vector>());
  const auto &points2f_1 = to_points(keypoints_1.values_matched(keypoints_0) |
                                     std::ranges::to<std::vector>());
...
auto zenslam::utils::triangulate_keylines(
    const map<keyline> &keylines_0, const map<keyline> &keylines_1,
    const cv::Matx34d &projection_0, const cv::Matx34d &projection_1,
    const class options::slam &options,
  ...
  const auto &points2f_0_0 = keylines_0.values_matched(keylines_1) |
                             points_0() | std::ranges::to<std::vector>();
  const auto &points2f_0_1 = keylines_0.values_matched(keylines_1) |
                             points_1() | std::ranges::to<std::vector>();
  const auto &points2f_1_0 = keylines_1.values_matched(keylines_0) |
                             points_0() | std::ranges::to<std::vector>();
  const auto &points2f_1_1 = keylines_1.values_matched(keylines_0) |
                             points_1() | std::ranges::to<std::vector>();

  const auto &points3d_cv_0 = triangulate_points(points2f_0_0, points2f_1_0,
                                                 projection_0, projection_1);
  const auto &points3d_cv_1 = triangulate_points(points2f_0_1, points2f_1_1,
                                                 projection_0, projection_1);
  ...
  const auto &angles_0 = points3d_cv_0 |
                         epipolar_angles(translation_of_camera1_in_camera0) |
                         std::ranges::to<std::vector>();
  const auto &angles_1 = points3d_cv_1 |
                         epipolar_angles(translation_of_camera1_in_camera0) |
                         std::ranges::to<std::vector>();
  ...
  for (auto i = 0; i < points3d_cv_0.size(); ++i) {
    ...
    auto angle = norm_prod < 1e-12
                     ...
                     : std::abs(std::acos(std::clamp(
                           vector_0.dot(vector_1) / norm_prod, -1.0, 1.0))) *
                           180 / CV_PI;
    ang.push_back(angle);
  }
  ...
  for (auto i = 0; i < points3d_cv_0.size(); ++i) {
    if (points3d_cv_0[i].z > 1 && points3d_cv_1[i].z > 1 &&
      ...
      lines3d.emplace_back(std::array{points3d_cv_0[i], points3d_cv_1[i]},
                           indices_0[i]);
    }
  }
...
auto zenslam::utils::triangulate_points(
    ...
    const std::vector<cv::Point2f> &points2f_1, const cv::Matx34d &projection_0,
  ...
  cv::Mat points4d{};
  cv::triangulatePoints(projection_0, projection_1, points2f_0, points2f_1,
                        points4d);
...
void zenslam::utils::umeyama(const std::vector<cv::Point3d> &src,
