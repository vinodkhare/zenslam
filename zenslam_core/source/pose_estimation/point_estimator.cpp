...
{
    auto point_estimator::estimate_3d2d(const std::map<size_t, point3d>& map_points_0, const std::map<size_t, keypoint>& map_keypoints_1) const -> std::optional<pose_data>
    {
        ...
        for (auto idx : indices) { if (!inliers_set.contains(idx)) out.outliers.push_back(idx); }
    ...
    auto point_estimator::estimate_3d3d(const std::map<size_t, point3d>& map_points_0, const std::map<size_t, point3d>& map_points_1) const -> std::optional<pose_data>
    {
        ...
        SPDLOG_DEBUG
        (
            "3D-3D points: {} correspondences, {} inliers",
            points_0.size(),
            inliers.size()
        );
    ...
    ) const -> std::optional<pose_data>
    {
        ...
        for (size_t i = 0; i < indices.size(); ++i)
        {
            if (inlier_mask[i]) out.inliers.push_back(indices[i]);
            else out.outliers.push_back(indices[i]);
        }
        ...
        SPDLOG_DEBUG
        (
            "2D-2D points: {} correspondences, {} inliers, scale={:.3f}",
            points0.size(),
            out.inliers.size(),
            s
        );
