...
namespace zenslam
{
    matcher::matcher(const slam_options& opts, const bool is_binary) : _options(opts) { _matcher = utils::create_matcher(_options, is_binary); }
    ...
    auto matcher::match_keypoints(const map<keypoint>& keypoints_0, const map<keypoint>& keypoints_1) const -> std::vector<cv::DMatch>
    {
        ...
        if (use_ratio_test)
        {
            ...
            {
                if (knn.size() == 2 && knn[0].distance < _options.matcher_ratio * knn[1].distance)
                {
    ...
    auto matcher::match_keypoints(const std::vector<keypoint>& keypoints_0, const std::vector<keypoint>& keypoints_1) const -> std::vector<cv::DMatch>
    {
        ...
        std::vector<size_t>  rows_0;
        std::vector<size_t>  rows_1;
        std::vector<cv::Mat> descriptors_0_list;
        std::vector<cv::Mat> descriptors_1_list;
        rows_0.reserve(keypoints_0.size());
        rows_1.reserve(keypoints_1.size());
        descriptors_0_list.reserve(keypoints_0.size());
        descriptors_1_list.reserve(keypoints_1.size());

        for (size_t i = 0; i < keypoints_0.size(); ++i)
        {
            if (keypoints_0[i].descriptor.empty())
            {
                continue;
            }

            rows_0.emplace_back(i);
            descriptors_0_list.emplace_back(keypoints_0[i].descriptor);
        }

        for (size_t i = 0; i < keypoints_1.size(); ++i)
        {
            if (keypoints_1[i].descriptor.empty())
            {
                continue;
            }

            rows_1.emplace_back(i);
            descriptors_1_list.emplace_back(keypoints_1[i].descriptor);
        }

        if (descriptors_0_list.empty() || descriptors_1_list.empty())
        {
            return matches_new;
        }
        ...
        if (matches.size() >= 8)
        {
            ...
            std::vector<uchar> inlier_mask;
            cv::findFundamentalMat(points_l, points_r, cv::FM_RANSAC, _options.epipolar_threshold, 0.99, inlier_mask);
        ...
        for (const auto& match : matches)
        {
            const auto& keypoint_l = keypoints_0[rows_0[match.queryIdx]];
            const auto& keypoint_r = keypoints_1[rows_1[match.trainIdx]];
