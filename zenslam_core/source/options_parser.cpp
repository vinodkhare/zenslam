...
namespace zenslam
{
    ...
    all_options options_parser::load_from_node(const YAML::Node& root)
    {
        ...
        if (root["log_level"])
        {
            const auto level_str = root["log_level"].as<std::string>();
            if (const auto level = magic_enum::enum_cast<spdlog::level::level_enum>(level_str))
            {
        ...
        if (root["verb"])
        {
            const auto verb_str = root["verb"].as<std::string>();
            if (const auto v = magic_enum::enum_cast<zenslam::verb>(verb_str))
            {
                opts.verb_ = v.value();
            }
        }
