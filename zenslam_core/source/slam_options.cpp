...
namespace zenslam
{
    ...
    void slam_options::validate() const
    {
        ...
        if (lba.value().window_size < 2)
            throw std::invalid_argument("slam.lba.window_size must be >= 2");
        if (lba.value().max_iterations < 1)
            throw std::invalid_argument("slam.lba.max_iterations must be >= 1");
        if (lba.value().huber_threshold < 0.0)
            throw std::invalid_argument("slam.lba.huber_threshold must be >= 0");
    }
