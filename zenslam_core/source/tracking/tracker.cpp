...
namespace zenslam
{
    ...
    auto tracker::track(const frame::tracked& frame_0, const frame::processed& frame_1) const -> frame::tracked
    {
...

        {
            ...
            std::jthread thread_0{
                [&]()
                {
                    ...
                    if (_detection.use_parallel_detector) { keypoints_0_detected = _detector.detect_keypoints_par(frame_1.undistorted[0], keypoints_0); } else
                    {
                    ...
                    auto keylines_0_detected = _detector.detect_keylines(frame_1.undistorted[0], keylines_0, _triangulation.keyline_mask_margin);
            ...
            std::jthread thread_1{
                [&]()
                {
                    ...
                    if (_detection.use_parallel_detector) { keypoints_1_detected = _detector.detect_keypoints_par(frame_1.undistorted[1], keypoints_1); } else
                    {
                    ...
                    // Detect new keylines (avoiding areas where tracked keylines exist)
                    auto keylines_1_detected = _detector.detect_keylines(frame_1.undistorted[1], keylines_1, _triangulation.keyline_mask_margin);
        ...
        // Descriptor matching for new keypoints when KLT tracking is weak
        {
            ...
            const auto ratio_left    = prev_left > 0 ? static_cast<double>(keypoints_0_tracked_count) / static_cast<double>(prev_left) : 1.0;
            const auto ratio_right   = prev_right > 0 ? static_cast<double>(keypoints_1_tracked_count) / static_cast<double>(prev_right) : 1.0;
            const auto tracked_ratio = std::min(ratio_left, ratio_right);
            ...
            if (tracked_ratio < _tracking.klt_min_tracked_ratio)
            {
                ...
                for (const auto& match : matches_left) { if (!keypoints_0.contains(match.queryIdx)) { filtered_left.emplace_back(match); } }

                std::vector<cv::DMatch> filtered_right;
                filtered_right.reserve(matches_right.size());
                for (const auto& match : matches_right) { if (!keypoints_1.contains(match.queryIdx)) { filtered_right.emplace_back(match); } }
        ...
        // Cross-camera (stereo) keyline tracking
        {
            ...
            auto keylines_1_tracked = track_keylines(frame_1.pyramids[0], frame_1.pyramids[1], keylines_0_untracked);
            ...
            auto keylines_0_tracked = track_keylines(frame_1.pyramids[1], frame_1.pyramids[0], keylines_1_untracked);
    ...
    auto tracker::assign_landmark_indices(std::vector<keyline>& keylines, const line3d_cloud& lines3d, double max_descriptor_distance) -> void
    {
        if (keylines.empty() || lines3d.empty()) { return; }
        ...
        {
            if (line3d.descriptor.empty()) { continue; }
