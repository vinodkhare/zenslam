...
namespace zenslam
{
    tracker::tracker(calibration calib, slam_options opts, frame::system& system) :
        ...
        _detector(detector::create(opts.detection)), _system(system)
    {
    }
    ...
    auto tracker::assign_landmark_indices(std::vector<keyline>& keylines, const line3d_cloud& lines3d, double max_descriptor_distance) const -> void
    {
    ...
    auto tracker::track(const frame::tracked& frame_0, const frame::processed& frame_1) const -> frame::tracked
    {
        ...
        // Cross-camera (stereo) keyline tracking
        {
            ...
            for (const auto& [index, keyline_0] : keylines_0)
            {
                if (!keylines_1.contains(index))
                {
                    keylines_0_untracked.emplace(index, keyline_0);
                }
            }
            ...
            for (const auto& [index, keyline_1] : keylines_1)
            {
                if (!keylines_0.contains(index))
                {
                    keylines_1_untracked.emplace(index, keyline_1);
                }
            }
