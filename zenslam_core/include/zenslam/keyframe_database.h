...
namespace zenslam
{
    ...
    public:
        ...
        [[nodiscard]] auto get(size_t id) const -> frame::estimated;
        [[nodiscard]] auto last() const -> frame::estimated;
        [[nodiscard]] auto recent(size_t max_count) const -> std::vector<frame::estimated>;

        auto add(const frame::estimated& frame) -> const frame::estimated&;
        auto update_pose(size_t id, const cv::Affine3d& pose) -> void;
    ...
    private:
        ...
        static auto compute_shared(const frame::estimated& a, const frame::estimated& b) -> size_t;
    };
}
